...
Fortune_Dow <- dplyr::right_join(Dow_Jones, Fortune500, by="YEAR") %>% select(YEAR, COMPANY, RANK, Open_, Close_) %>% filter(YEAR %in% c("1980", "1990", "2000"), RANK <= 10) %>% View

#All else fails commenting out Fortune with Country combo
#Fortune_Country <- dplyr::right_join(Fortune500, Country, by="YEAR") %>% select(YEAR, North_America, PROFIT_IN_MILLIONS_, COMPANY, RANK) %>% filter(YEAR %in% c("1980", "1990", "2000"), RANK <= 10) %>% View
